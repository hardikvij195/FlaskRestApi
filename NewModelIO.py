...
def vectorization(df, columns):
    ...
    if column_name in ['Drinking', 'Smoking', 'Gender']:
        ...
        print('entering else')
        new_df = df.drop(column_name, axis=1).join(df[column_name].str.join(
            '|').str.get_dummies(), lsuffix='_left', rsuffix='_right')
...
def PredCluster(user_data, user_n):
    ...
    new_cluster = pd.DataFrame(scaler.fit_transform(
        new_cluster), index=new_cluster.index, columns=new_cluster.columns)
    ...
    likely_users = new_list[[user_n]].sort_values(
        by=[user_n], axis=0, ascending=False)[1:4]
